...
async def setup(bot: commands.Bot):
    """
    Setup function to initialize the CourseManagement cog.
...
class CourseManagement(commands.Cog):
    """
    A class representing commands for managing courses.
            ...
            if "select_role_" in interaction.data["custom_id"]:
                try:
                    ...
                    if "select_role_class_" in interaction.data["custom_id"]:
                        name = (
                            re.search(
                                r"^select_role_class_(\w+_\d+(?:_\(.*\))?)_.*",
                    ...
                    # If not class, search for greedy role
                        ...
                        name = (
                            re.search(
                                r"^select_role_(.*)", interaction.data["custom_id"]
                            )
                            .group(1)
                            .replace("_", " ")
                        )
                ...
                if role is not None:
                    ...
                    if role in interaction.user.roles:
                        ...
                        await interaction.response.send_message(
                            f"The {role.mention} role has been removed from you.",
                            ephemeral=True,
                        )
                    # If user does not have role, add it
                        ...
                        await interaction.response.send_message(
                            f"The {role.mention} role has been given to you.",
                            ephemeral=True,
                        )
                else:
                    await interaction.response.send_message(
                        "Could not find role to add", ephemeral=True
                    )
    ...
    async def build_courses(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "Please send CSV if you intend to use one. If you do not intend to use one, the cached CSV will be used."
        )
        csv_filepath = f"role_lists/roles_{interaction.guild.id}.csv"

        csv = await interaction.client.wait_for(
        ...
        for i, role_name in enumerate(role_names):
            ...
            )  # sets category to private
            await category.set_permissions(
                role, read_messages=True
            )  # allow role to see category
                ...
                # Create voice channel(s)
                    ...
                    if member_count == 0:
                        ...
                        await category.create_voice_channel(
                            channel_name, user_limit=int(member_count)
                        )
    ...
    async def rolemenu_callback(self, interaction: discord.Interaction, prefixes: list):
        ...
        if confirmation_message:
            confirmation_message = (
                "**The following role(s) could not be found:**\n" + confirmation_message
            )
        ...
        for prefix in prefixes:
            ...
            for i in range(len(role_names)):
                ...
                if re.match(prefix, category_names[i]):
                    ...
                    this_button = discord.ui.Button(
                        label=f"{category_names[i]} - {long_names[i]}",
                        style=discord.ButtonStyle.gray,
                        custom_id=f"select_role_class_{name}_{i}",
                    )
    ...
    async def build_role_menu(self, interaction: discord.Interaction):
        ...
        select = discord.ui.Select(
            placeholder="Select an option", max_values=len(options), options=options
        )
        ...
        await interaction.response.send_message(
            "Select one or more prefixes", view=option_view
        )
    ...
    async def create_role_button(
        ...
        if "http" in role_name:
            this_button = discord.ui.Button(
                label=button_name, style=discord.ButtonStyle.url, url=role_name
            )
        else:
            this_button = discord.ui.Button(
                label=button_name,
                style=discord.ButtonStyle.gray,
                custom_id=f"select_role_{role_name.replace(' ', '_')}",
            )
        if emoji != "None":
        ...
        try:
            ...
            logger.warning(
                f"{interaction.user} tried creating the '{button_name}' button for role '{role_name}' role in #{interaction.channel} but failed because emoji did not exist"
            )
        else:
            logger.success(
                f"{interaction.user} created the '{role_name}' role and '{button_name}' button in #{interaction.channel}"
            )
