...
async def setup(bot):
    ...
    if not os.path.exists(db_path):
        ...
        logger.info("Database already exists", discord=False)
...
class Checkin(commands.Cog):
    ...
    async def on_interaction(self, interaction: discord.Interaction):
        ...
        if (
            ...
            and interaction.data["custom_id"] is not None
            ...
            if "checkin_checkin_btn" in interaction.data["custom_id"]:
                ...
                await interaction.response.send_message(
                    "You have checked in!", ephemeral=True
                )
            elif "checkedin_pomo_btn" in interaction.data["custom_id"]:
                modal = discord.ui.Modal(
                    title="Pomodoro Creation",
                    custom_id=f"checkedin_pomo_create_{interaction.message.id}",
                )
                modal.add_item(
                    discord.ui.TextInput(
                        label="What issue are you working on?", required=True
                    )
                )
                ...
                time = str(await get_time_epoch())
                pomo_reason = interaction.data["components"][0]["components"][0][
                    "value"
                ]
                ...
                message_id = int(
                    interaction.data["custom_id"].replace("checkedin_pomo_create_", "")
                )
                ...
                await interaction.response.send_message(
                    "You have started a pomodoro. I will check with you in 20 minutes",
                    ephemeral=True,
                )
                ...
                timesheet = update_timesheet(
                    conn, time_id, interaction.user.id, time_in, time_out, total_time
                )
                ...
                if timesheet:
                    ...
                    await interaction.response.send_message(
                        f"You have now been clocked out. Total time: **{await get_string_from_epoch(total_time)}**",
                        ephemeral=True,
                    )
                else:
                    await interaction.response.send_message(
                        "Error while checking out", ephemeral=True
                    )
            elif (
                "pomo_done_btn" in interaction.data["custom_id"]
                or "pomo_not_done_btn" in interaction.data["custom_id"]
            ):
                conn = create_connection("cse_discord.db")
                ...
                if pomo is not None:
                    ...
                    pomodoro = update_pomodoro(
                        ...
                        total_time,
                        str(
                            3 if "pomo_done_btn" in interaction.data["custom_id"] else 2
                        ),
                        pomo[7],
                    )
                    ...
                    if True is not None:
                        ...
                        await update_view(interaction, Checkin.checked_in_view())
                        await interaction.response.send_message(
                            f"You have now completed your pomodoro. Total time: **{await get_string_from_epoch(total_time)}**",
                            ephemeral=True,
                        )
                    else:
                        await interaction.response.send_message(
                            f"Error! Unable to close pomodoro", ephemeral=True
                        )
                ...
                if pomo is not None:
                    modal = discord.ui.Modal(
                        title="Pomodoro Blocked", custom_id="pomo_blocked_create"
                    )
                    modal.add_item(
                        discord.ui.TextInput(
                            label="What issue are you having?", required=True
                        )
                    )
                ...
                if pomo is not None:
                    pomodoro = update_pomodoro(
                        ...
                        None,
                        None,
                        str(1 if pomo[7] is None else int(pomo[7]) + 1),
                    )
                    ...
                    if help is not None:
                        await interaction.response.send_message(
                            f"Your issue has been recorded", ephemeral=True
                        )
                    else:
                        await interaction.response.send_message(
    ...
    def checkin_view(self):
        ...
        view = View()
        checkin_button = discord.ui.Button(
            label="Check-in",
            style=discord.ButtonStyle.green,
            custom_id="checkin_checkin_btn",
        )
        view.add_item(checkin_button)

        return view

    def checked_in_view(self):
        ...
        checkout_button = discord.ui.Button(
            ...
            custom_id="checkedin_checkout_btn",
        )
        pomo_button = discord.ui.Button(
            label="Pomodoro",
            style=discord.ButtonStyle.blurple,
            custom_id="checkedin_pomo_btn",
        )
    ...
    def pomo_view(self):
        ...
        done_button = discord.ui.Button(
            label="Done", style=discord.ButtonStyle.green, custom_id="pomo_done_btn"
        )
        blocked_button = discord.ui.Button(
            label="Help/Blocked",
            style=discord.ButtonStyle.red,
            custom_id="pomo_blocked_btn",
        )
        not_done_button = discord.ui.Button(
            label="Not Done",
            style=discord.ButtonStyle.secondary,
            custom_id="pomo_not_done_btn",
        )
    ...
    @check_in_group.command(
        name="register", description="Register for checkin/timesheets!"
    )
    async def checkin_register(self, interaction: discord.Interaction):
        ...
        if status == "User already exists":
            await interaction.response.send_message(
                "ERROR: You are already registered", ephemeral=True
            )
            return
        elif status == "Could not connect to database":
            await interaction.response.send_message(
                "ERROR: Unable to connect to database", ephemeral=True
            )
            return
        elif status == "Error":
            await interaction.response.send_message(
                "Error registering for checkin", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            "A DM has been sent to you", ephemeral=True
        )

    @check_in_group.command(
        name="clear",
        description="Clear your messages from the bot and reset timesheets",
    )
    async def checkin_clear(self, interaction: discord.Interaction):
        ...
        if channel.id == interaction.channel_id:
            ...
            await interaction.followup.send(
                "Up to 10 direct messages cleared. If you were clocked in, you've also been clocked out",
                ephemeral=True,
            )
        else:
            await interaction.response.send_message(
                "Cannot clear outside of your DMs", ephemeral=True
            )

    async def clear_checkin_messages(
        ...
        if pomo_id is not None and time_id is not None:
            ...
            update_pomodoro(
                conn, pomo_id, time_id, "", time_start, time_end, total_time, 2
            )

        conn.close()

        # Send new view
        await channel.send(view=Checkin.checkin_view(self))
    ...
    async def check_pomodoros(self):
        ...
        if pomodoros is not None:
            for pomodoro in pomodoros:
                time = str(await get_time_epoch())
                if float(time) >= (float(pomodoro[3]) + (20 * 60)) and (
                    pomodoro[6] == 0 or pomodoro[6] == None
                ):
                    ...
                    if user is not None:
                        ...
                        await channel.send(
                            "According to my watch, 20 minutes has passed. How are things going?"
                        )
                elif (
                    float(time) >= (float(pomodoro[3]) + (8 * 60 * 60))
                    and pomodoro[6] == 1
                ):
            ...
            for timesheet in timesheets:
                ...
                if time >= (float(timesheet[2]) + (8 * 60 * 60)):
                    ...
                    if user is not None:
                        ...
                        channel = await user.create_dm()
                        await change_checkin_status(
                            self.bot, user.id, user.display_name, "checkout"
                        )
                        await Checkin.clear_checkin_messages(self, channel, user)

    @check_in_group.command(
        name="report",
        description="Generate Report of the user's timesheet. Date Format should be in MM-DD-YYYY",
    )
    async def get_report(
        ...
        if start_time is None and end_time is None:
            ...
            all_records, total_hours, complete_pomodoros = get_user_report(
                conn, interaction.user.id, new_start_time, new_end_time
            )
            ...
            if all_records is None or complete_pomodoros is None or total_hours is None:
                ...
                response_message = result_parser(
                    all_records, total_hours, complete_pomodoros
                )
            ...
            await interaction.response.send_message(
                "Please provide both dates for a given range or leave empty for your last pay period",
                ephemeral=True,
            )
    ...
    @check_in_group.command(
        name="report-monthly",
        description="Generate report of multiple user for the month. Date Format should be in MM-DD-YYYY",
    )
    @app_commands.default_permissions(administrator=True)
    async def get_montly_report(
        self,
        interaction: discord.Interaction,
        role: discord.Role,
        start_time: str = None,
        ...
        if start_time is None and end_time is None:
            for member in role.members:
                ...
                new_end_time = time.time()
                all_records, total_hours, complete_pomodoros = get_user_report(
                    conn, member.id, new_start_time, new_end_time
                )
            ...
            await interaction.followup.send(
                ...
                file=discord.File("assets/admin_user_report.txt"),
                ephemeral=True,
            )
        elif start_time is not None and end_time is not None:
            for member in role.members:
            ...
            await interaction.followup.send(
                "Please provide both dates for a given range or leave empty for the last pay period",
                ephemeral=True,
            )
