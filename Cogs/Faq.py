        ...
        if message.channel.name in self.channel_names:
            if "?" in message.content:
                ...
                if message.content in keywords:
                    question = df.loc[
                        df["keywords"] == message.content, "questions"
                    ].values[0]
                    answer = df.loc[
                        df["keywords"] == message.content, "answers"
                    ].values[0]
    ...
    async def faq(self, interaction: discord.Interaction):
        ...
        if interaction.channel.name in self.channel_names:
            ...
            await interaction.response.send_message(
                "FAQ has been disabled for this channel!"
            )

            # Logging
            logger.info(
                f"{interaction.user} has disabled FAQ for the {interaction.channel} channel"
            )

        # If the channel is not in the list add it to the end
        else:
            await interaction.response.send_message(
                "FAQ has been enabled for this channel!"
            )
            self.channel_names.append(interaction.channel.name)

            # Logging
            logger.info(
                f"{interaction.user} has enabled FAQ for the {interaction.channel} channel"
            )
