...
class Gourmet(commands.Cog):
    ...
    class GourmetMenu(View):
        ...
        @discord.ui.button(
            label="Random", style=discord.ButtonStyle.blurple, emoji="\U0001f3b1"
        )
        async def random(
            ...
            if not len(self.normal_restaurant) == 0:
                await interaction.response.edit_message(
                    content="**" + self.normal_restaurant.pop() + "**", view=self
                )
            else:
                await interaction.response.edit_message(
                    content="**__You have run out of restaurants. You are going to arbys.__**",
                    view=self,
                )
        ...
        async def add(
            ...
            await interaction.response.edit_message(
                content="**__Enter a restaurant to add to the list.__**", view=self
            )
            ...
            for rest in self.main_list:
                if msg.content == rest.casefold():
                    await interaction.message.edit(
                        content=f"**__Error: {msg.content} already is in the restaurant list.__**",
                        view=self,
                    )
                    return
            ...
            await interaction.message.edit(
                content=f"**__{msg.content} has been added to the list.__**"
            )
        ...
        async def remove(
            ...
            await interaction.response.edit_message(
                content="**__Enter a restaurant to remove from the list.__**", view=self
            )
            ...
            for rest in self.main_list:
                if msg.content == rest.casefold():
                    ...
                    await interaction.message.edit(
                        content=f"**__{msg.content} has been removed.__**", view=self
                    )
                    return
            await interaction.message.edit(
                content=f"**__Error: {msg.content} is not in the list.__**", view=self
            )

            self.normal_restaurant = self.main_list

        @discord.ui.button(
            label="Vegan", style=discord.ButtonStyle.blurple, emoji="\U0001f96c"
        )
        async def vegan(
            self, interaction: discord.Interaction, button: discord.ui.Button
        ):
            ...
            if not len(self.vegan_restaurant) == 0:
                await interaction.response.edit_message(
                    content="**" + self.vegan_restaurant.pop() + "**", view=self
                )
            else:
                await interaction.response.edit_message(
                    content="**__You have run out of restaurants. You are going to arbys.__**",
    ...
    @app_commands.command(
        description="Sends a message with buttons to give you options on possible food choices"
    )
    async def feedme(self, interaction: discord.Interaction):
        ...
        logger.info(f"{interaction.user} ran /feedMe in `#{interaction.channel}`")
