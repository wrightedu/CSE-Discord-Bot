...
class Listeners(commands.Cog):
    ...
    async def on_message_edit(self, before, after):
        ...
        if isinstance(before.channel, discord.TextChannel):
            ...
            message_log_channel = discord.utils.get(
                before.guild.channels, name="message-log"
            )
            ...
            if message_log_channel and before.author != self.bot.user:
                ...
                embed = discord.Embed(
                    title=f"Message Edited at [{str(datetime.datetime.now())[:-7]}]",
                    color=0x00FF00,
                )
                embed.add_field(
                    name="Original Message", value=f'"{before.content}"', inline=False
                )
                embed.add_field(
                    name="Edited Message", value=f'"{after.content}"', inline=False
                )
                embed.set_footer(
                    text=f"Message edited by {after.author} in {before.channel.name}"
                )
    ...
    async def on_message_delete(self, message):
        ...
        if isinstance(message.channel, discord.TextChannel):
            # Get the channel to send the message to
            message_log_channel = discord.utils.get(
                message.guild.channels, name="message-log"
            )
            ...
            if message_log_channel and message.author != self.bot.user:
                # Create an embed to log the message
                embed = discord.Embed(
                    title=f"Message Deleted at [{str(datetime.datetime.now())[:-7]}]",
                    color=0xFF0000,
                )
                embed.add_field(
                    name="Message Deleted", value=f'"{message.content}"', inline=False
                )
                embed.set_footer(
                    text=f"Message deleted by {message.author} in {message.channel.name}"
                )
